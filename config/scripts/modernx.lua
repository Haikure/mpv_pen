...
local function touch_extra_buttons() end
local touch_zoom_active = false
...
local user_opts = {
    ...
    keep_on_pause = true,      -- disable OSC hide timeout when paused
    ...
    vid_scale = true,               -- scale osc with the video
    scale_windowed = 2.75,           -- osc scale factor when windowed
    scale_fullscreen = 2.75,         -- osc scale factor when fullscreen
    scale_forced_window = 2.75,      -- osc scale factor when forced (no video, like music files)
...
local language = {
    ['en'] = {
        welcome = '将文件或 URL 拖放到此处播放', -- 这段文本在 mpv 启动时显示
        off = '关闭',
        na = '不可用',
        none = '无可用选项',
        video = '视频',
...
local osc_styles = {
    ...
    osc_color_convert(user_opts.window_controls_color) .. "&\\3c&H0&\\fs18\\fnmpv-osd-symbols}",
...
function checktitle()
    ...
    if (mp.get_property("filtered-metadata/by-key/Album_Artist") and mp.get_property("filtered-metadata/by-key/Artist")) then
        if (mp.get_property("filtered-metadata/by-key/Album_Artist") ~= mp.get_property("filtered-metadata/by-key/Artist")) then
            artist = mp.get_property("filtered-metadata/by-key/Artist") ..
            ", " .. mp.get_property("filtered-metadata/by-key/Album_Artist")
            tempartistclicktext = "Contributing artists: " ..
            mp.get_property("filtered-metadata/by-key/Artist") ..
            "\\NAlbum arist: " .. mp.get_property("filtered-metadata/by-key/Album_Artist")
        end
    end
...
function exec_async(args, callback)
    -- 防止 LD_LIBRARY_PATH 污染子进程
    if args and args[1] ~= "env" then
        local new_args = {"env", "-u", "LD_LIBRARY_PATH"}
        for i = 1, #args do
    ...
    if is_url(path) and path or nil then
        ...
        if path:find('youtu%.?be') and (user_opts.show_description or user_opts.title_youtube_stats) then
            ...
            if match then
                ...
                if videoID then
                    exec_async({ "curl", "https://returnyoutubedislikeapi.com/votes?videoId=" .. videoID },
                        process_dislikes)
                else
...
function process_vid_stats(success, result, error)
    ...
    if (state.dislikes == "") then
        ...
        string.gsub(result.stdout, '\r', '\\N'):gsub("\n", "\\N")
    ...
    if (state.localDescriptionClick:match('Views: (%d+)')) then
        state.localDescriptionClick = state.localDescriptionClick:gsub(state.localDescriptionClick:match('Views: (%d+)'),
            add_commas_to_number(state.localDescriptionClick:match('Views: (%d+)')))
    end
    if (state.localDescriptionClick:match('Likes: (%d+)')) then
        state.localDescriptionClick = state.localDescriptionClick:gsub(state.localDescriptionClick:match('Likes: (%d+)'),
            add_commas_to_number(state.localDescriptionClick:match('Likes: (%d+)')))
    end
    if (state.localDescriptionClick:match('Comments: (%d+)')) then
        ...
        state.localDescriptionClick:match('Comments: (%d+)'),
            add_commas_to_number(state.localDescriptionClick:match('Comments: (%d+)')))
    end
    ...
    if false then
        state.localDescriptionClick = state.localDescriptionClick:gsub("Views:", icons.emoticon.view):gsub("Comments:",
            icons.emoticon.comment):gsub("Likes:", icons.emoticon.like):gsub("Dislikes:", icons.emoticon.dislike)                                                                                                                                                                                                                                                                                                                                                                     -- replace with icons
    end
...
function touch_extra_buttons()
    ...
    local x2 = x1 + size
    local reset_y2 = reset_y1 + size
    local zoom_y2 = zoom_y1 + size

    add_area('touch-extra-input', x1, reset_y1, x2, zoom_y2)

    ne = new_element('touch_reset_bg', 'box')
    lo = add_layout('touch_reset_bg')
    lo.geometry = { x = x1, y = reset_y1, an = 7, w = size, h = size }
    lo.style = "{\\blur0\\bord0\\shad0\\1c&H202020&}"
    lo.layer = 20
    lo.alpha[1] = 112
    lo.box.radius = radius

    ne = new_element('touch_zoom_bg', 'box')
    lo = add_layout('touch_zoom_bg')
    lo.geometry = { x = x1, y = zoom_y1, an = 7, w = size, h = size }
    lo.style = "{\\blur0\\bord0\\shad0\\1c&H202020&}"
    lo.layer = 20
    lo.alpha[1] = 112
    lo.box.radius = radius

    ne = new_element('touch_reset', 'button')
    ne.content = '↺'
    ne.eventresponder['mbtn_left_up'] = function()
        mp.commandv('script-message-to', 'touch_control', 'touch_zoom_reset')
    end
    ...
    ne = new_element('touch_zoom', 'button')
    ne.content = '+'
    ne.eventresponder['mbtn_left_up'] = function()
        mp.commandv('script-message-to', 'touch_control', 'touch_zoom_in')
    end
...
layouts["original"] = function()
    ...
    (user_opts.seekbar_between_timers and 25 or 0), an = 1, w = osc_geo.w - 50, h = 35 }
    lo = add_layout("title")
    lo.geometry = geo
    ...
    lo.geometry = { x = osc_geo.w - 25, y = refY - 84, an = 9, w = 35 + (state.tc_ms and 30 or 0) +
    (show_remhours and 25 or 0), h = 20 }
    lo.style = osc_styles.time
    ...
    if user_opts.show_chapter_title then
        ...
        lo.geometry = { x = 77 + (state.tc_ms and 25 or 0) + (show_hours and 16 or 0), y = refY - 84, an = 7, w = osc_geo
        .w - 200 - ((show_hours or state.tc_ms) and 60 or 0), h = 20 }
    ...
    lo.geometry = { x = osc_geo.w - 58, y = refY - 40, an = 5, w = 76, h = 32 }
    lo.style = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.side_buttons_color) .. "&\\3c&HFFFFFF&\\fs22\\fn" .. user_opts.font .. "}"
    ...
    if screenshot_button then
        ...
        lo.geometry = { x = osc_geo.w - 217 + (loop_button and 0 or 45) + (ontop_button and 0 or 45) +
        (info_button and 0 or 45), y = refY - 40, an = 5, w = 24, h = 24 }
        lo.style = osc_styles.control_3
    ...
    if user_opts.download_button then
        ...
        lo.geometry = { x = osc_geo.w - 262 + (loop_button and 0 or 45) + (ontop_button and 0 or 45) +
        (info_button and 0 or 45) + (screenshot_button and 0 or 45), y = refY - 40, an = 5, w = 24, h = 24 }
        lo.style = osc_styles.control_3
...
local function osc_init()
    ...
    local function speed_label(speed)
        if math.abs(speed - 0.75) < 0.01 then return '0.75x' end
        if math.abs(speed - 1.0) < 0.01 then return '1x' end
        if math.abs(speed - 1.5) < 0.01 then return '1.5x' end
        if math.abs(speed - 1.75) < 0.01 then return '1.75x' end
        if math.abs(speed - 2.0) < 0.01 then return '2x' end
        return string.format('%.2fx', speed)
    end
    
    ne.content = function()
        return speed_label(mp.get_property_number('speed', 1) or 1)
    end
    ...
    ne.eventresponder['mbtn_left_up'] =
        function()
            ...
            local speeds = { 0.75, 1.0, 1.5, 1.75, 2.0 }
            local new_speed = speeds[1]
    
            for i = 1, #speeds do
                if math.abs(speed - speeds[i]) < 0.01 then
                    new_speed = speeds[i % #speeds + 1]
                    break
                elseif speed < speeds[i] then
                    new_speed = speeds[i]
                    break
                end
            end
            ...
            mp.osd_message(speed_label(new_speed), 0.8)
        end
    ...
    state.is_URL
    ne.tooltip_style = osc_styles.tooltip
    ne.tooltipF = function() return state.downloading and (texts.downloading .. "...") or
        (texts.download .. " (" .. state.file_size_normalized .. ")") end
    ne.eventresponder["mbtn_left_up"] = function()
...
function show_osc()
    ...
    mp.set_property_bool("user-data/modernx/touch-shield", true)
...
function hide_osc()
    mp.set_property_bool("user-data/modernx/touch-shield", false)
...
function osc_visible(visible)
    mp.set_property_bool("user-data/modernx/osc-visible", visible)
    mp.commandv("script-message-to", "touch_control", "modernx_osc_visible", visible and "yes" or "no")
...
function pause_state(name, enabled)
    ...
    -- 播放结束会触发 pause；缩放期间不要因此重新启用 OSC，避免两套缩放按钮叠加。
    if touch_zoom_active then
        request_tick()
        return
    end
...
local function render()
    ...
    if osc_param.areas['touch-extra-input'] then
        for _, cords in ipairs(osc_param.areas['touch-extra-input']) do
            if state.osc_visible then
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, 'touch-extra-input')
                mp.enable_key_bindings('touch-extra-input')
            else
                mp.disable_key_bindings('touch-extra-input')
            end
        ...
        set_virt_mouse_area(0, 0, 0, 0, 'touch-extra-input')
        mp.disable_key_bindings('touch-extra-input')
    end
...
mp.observe_property('loop-file', 'native',
    function(name, val)                                                                                                                                                                                                                                                             -- ensure compatibility with auto looping scripts (eg: a script that sets videos under 2 seconds to loop by default)
        state.looping = val ~= nil and val ~= false and val ~= "no" and val ~= 0
    end
)
...
mp.observe_property('speed', 'number', function() request_tick() end)
...
mp.register_script_message("touch_zoom_mode", function(v)
    if v == "yes" then
